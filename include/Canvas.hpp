...
namespace KTLib
{
    ...
    class Canvas
    {
        public:
            Canvas() = default;
            Canvas(int width, int height, Color col = Color::Transparent());
            Canvas(unsigned int* buffer, int width, int height);
            Canvas(Color** colors, int width, int height);
            Canvas(const std::vector<std::vector<Color>>& colors);
            Canvas(const Gradient& gradient, int width, int height);
            ...
            static Canvas* FromHBITMAP(HBITMAP hBitmap, int width = 0, int height = 0);
            HDC ToHDC(int width, int height) const;
            static Canvas* FromHDC(HDC hdc, int x, int y, int width, int height);
            HICON ToHICON(int width, int height) const;
            static Canvas* FromHICON(HICON hIcon);
            HCURSOR ToHCURSOR(int width, int height) const;
            static Canvas* FromHCURSOR(HCURSOR hCursor);
            static Canvas* FromHWND(HWND hWnd, int x, int y, int width, int height);
            ...
            void OverlayImage(const Canvas& overlay, int x, int y, double opacity = 1.0);
            void Emboss();
            void EdgeDetect();
            ...
            Canvas* Copy() const;
            Canvas* CopyRegion(int xmin, int ymin, int width, int height) const;
            ...
            Canvas Filter(const std::function<bool(const Color&)>& predicate) const;
            int Count(const Color& color) const;
            void Shuffle();
            void Clear();
            void Sort(const std::function<bool(const Color&, const Color&)>& compare);
            void AppendRight(const Canvas& other);
            void AppendBottom(const Canvas& other);
